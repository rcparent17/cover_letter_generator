...
def test_generate_letter_filename(generator, company):
    assert (
        generator._generate_letter_filename(company)
...
def test_generate_resume_filename(generator, company):
    assert (
        generator._generate_resume_filename(company)
        == "reilly_parent_comp1_freeloader_resume.pdf"
    )


def test_populate_template(generator, company):
    generator.template = "{COMPANY_NAME}#{COMPANY_LOCATION}#{JOB_TITLE}\n{REQUIREMENTS}\n{QUALIFICATIONS}"
    ...
    expected_qualifications = (
        "<li>i eat</li><li>i sleep</li><li>i will die</li><li>i will ascend</li>"
    )
    expected_populated_template = "\n".join(
        [
            "comp1#nowhere#freeloader",
